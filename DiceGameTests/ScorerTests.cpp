...
Scorer ScorerTests<Scorer>::CreateScorer()
{
    return Scorer();
}

OpenClosedScorer ScorerTests<OpenClosedScorer>::CreateScorer()
{
    ScoringRulesFactory rulesFactory;
    return OpenClosedScorer(rulesFactory.CreateRules());
}
...
TYPED_TEST(ScorerTests, GivenOneDie_WhenFiveIsThrown_ScoreShouldBe50)
{
    ...
    TypeParam scorer(this->CreateScorer());
